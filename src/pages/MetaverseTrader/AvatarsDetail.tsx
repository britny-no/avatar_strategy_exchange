...
const index = () => {
    ...
    return (
        ...
        <ContentHead>
            <p className="title">{t("metaverseTrader:avatars_detail")}</p>
            <br/>
            <br/>
            <br/>
        </ContentHead>
...
const List = ({info}) => {
    ...
    const t3911 = useTypedSelector((state) => state.stateReducer.t3911);
    const  { email } = useTypedSelector((state) => state.userReducer.data);

    const [loading, setLoading] = useState(false);
    const [pre_form, setPreForm] = useState({})
    const [form, setForm] = useState({})
    const [popup, setPopup] = useState<boolean | Record<string, string>>(false)
    const ob = {
    ...
    const reqSave = (e) => {
        ...
        const pre_data = pre_form[type]

        setLoading(true)
        socketService.sendToAgent({
            Header : {
                function : "D", termtype : "HTS", trcode : "t3911"
            }, 
            Input1 : {
                szStrategy, szCurNo,szStrategy_ID, szUserID : `000${email}`
            }
        });
        ...
        if(t3901 && t3901.Output2){
            ...
            <thead style={{ display: 'block', width: '100%' }}>
                <tr className="column" style={{ display: 'flex', width: '100%', flexFlow:"wrap" }}>
                    <th style={{ width: '35%' }}>{t("metaverseTrader:strategy")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:symbol")}</th>
                    <th style={{ width: '25%' }}>{t("metaverseTrader:strategy_name")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:stat")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:sell_buy")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:positions")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:p_l")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:config")}</th>
                    <th style={{ width: '20%' }}>{t("metaverseTrader:set")}</th>
                </tr>
            </thead>
            <tbody
                style={{
            ...
            {t3901.Output2.filter((v, k) => form[k] !== undefined).map((v, k) => {
                return <AcriveTrM className="row" key={k}>
                    <RowMiddleM style={{ width: '35%' }}>{v[0]}: {ob[v[0].trim()]}</RowMiddleM>
                    <RowMiddleM style={{ width: '20%' }}>{v[1]}</RowMiddleM>
                    <RowMiddleM style={{ width: '25%' }}>{v[2]}</RowMiddleM>
                    <RowMiddleM style={{ width: '20%' }}>LV {v[7]}</RowMiddleM>
                    <RowMiddleM style={{ width: '20%' }}>{v[5]}</RowMiddleM>
                    <RowMiddleM style={{ width: '20%' }}>{v[6]}</RowMiddleM>
                    <RowMiddleM style={{ width: '20%' }}>{v[8]}</RowMiddleM>
                    <RowMiddleM style={{ width: '20%' }}><button name={k} onClick={() => setPopup(v)}>{t("metaverseTrader:change")}</button></RowMiddleM>
        ...
        if(t3901 && t3901.Output2){
            ...
            <thead style={{ display: 'table', width: '100%' }}>
                <tr className="column">
                    ...
                    <th style={{ width: '13%' }}>{t("metaverseTrader:symbol")}</th>
                    <th style={{ width: '18%' }}>{t("metaverseTrader:strategy_name")}</th>
                    <th style={{ width: '8%' }}>{t("metaverseTrader:stat")}</th>
                    <th style={{ width: '10%' }}>{t("metaverseTrader:sell_buy")}</th>
                    <th style={{ width: '10%' }}>{t("metaverseTrader:positions")}</th>
                    <th style={{ width: '10%' }}>{t("metaverseTrader:p_l")}</th>
                    <th style={{ width: '8%' }}>{t("metaverseTrader:config")}</th>
                    <th style={{ width: '8%' }}>{t("metaverseTrader:set")}</th>
                </tr>
            </thead>
            <tbody
                style={{
            ...
            {t3901.Output2.filter((v, k) => form[k] !== undefined).map((v, k) => {
                ...
                return <AcriveTr className="row" key={k}>
                    <RowMiddle style={{ width: '15%' }}>{v[0]}: {ob[v[0].trim()]}</RowMiddle>
                    <RowMiddle style={{ width: '13%' }}>{v[1]}</RowMiddle>
                    <RowMiddle style={{ width: '18%' }}>{v[2]}</RowMiddle>
                    <RowMiddle style={{ width: '8%' }}>LV {v[7]}</RowMiddle>
                    <RowMiddle style={{ width: '10%' }}>{v[5]}</RowMiddle>
                    <RowMiddle style={{ width: '10%' }}>{v[6]}</RowMiddle>
                    <RowMiddle style={{ width: '10%' }}>{v[8]}</RowMiddle>
                    <RowMiddle style={{ width: '8%' }}><button name={k} onClick={() => setPopup(v)}>{t("metaverseTrader:change")}</button></RowMiddle>
                    <RowMiddle style={{ width: '8%' }}><button name={k} onClick={reqSave}>{t("metaverseTrader:run")}</button></RowMiddle>
                </AcriveTr>
            })}
            </tbody>
        </AssetTable>
    ...
    return (
        <>
            {popup ? <MyDetail setPopup={setPopup} popup={popup}/> :             <div style={{ display: 'flex', justifyContent: 'space-between', flexFlow: 'wrap' }}>
